...
bool checkAnalogData(AnalogData *data, uint32_t value, uint16_t recoverOffset)
{
	...
	if (value < data->lowerBound) {
		// current value to low, increment numBelowLimit if not max already
...
float inputToCelcius(uint32_t data)
{
	...
	float R_10k = 11500;	// 10k resistor measured resistance in Ohms (other element in the voltage divider)
	float R_25 = 50000;
